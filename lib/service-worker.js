...
module.exports = function (hook, preprocess) {
  ...
  function onMessage(event) {
    if (event.data === 'channel') {
      ...
      let hookWorkers = [];
      for (let i = 1; i < event.ports.length; i++) {
        hookWorkers.push({ worker: i - 1, port: event.ports[i], taskQueueSize: 0 });
      }
      preprocess.setHookWorkers(hookWorkers);
      responsePort.postMessage('channel');
    }
                  ...
                  return response.text().then(async function(result) {
                    ...
                    try {
                      if (noHook) {
                        ...
                        if (noHook) {
                          result = preprocess.setScriptHookProperty(result, url); // only for hook.min.js
                        }
                      }
                      if (!noHook) {
