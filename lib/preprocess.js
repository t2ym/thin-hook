  ...
  function setupHookWorkers() {
    if (hookWorkers.length === 0 && hook.parameters.hookWorker) {
      ...
      for (let i = 0; i < hookWorkerCount; i++) {
        let worker = {
          worker: new Worker(hook.parameters.hookWorker + '&service-worker-initiator=' + serviceWorkerInitiator),
        ...
        if (ast.__for__ &&
          ...
          case 'Identifier':
            if (!blockScope[ast.declarations[0].id.name]) {
              ...
              if (t) {
                template.property.name = 'S' + _hookPrefix + body.name;
              }
              else {
      ...
      if (ast.__hooked__) {
        ...
        if (_hookProperty) {
          ...
          if (_hookGlobal && Object.getPrototypeOf(functionScope) === globalScope) {
            ...
            if (t) {
              template.expression.left.property.name = 'S' + _hookPrefix + ast.id.name;
            }
            else {
    ...
    case 'CallExpression':
      if (ast.__hooked__) {
        ...
        if (ast.callee.type === 'Super') {
          ...
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse('_X_hookName_X_((Import,ImportSpecifier) => import(ImportSpecifier), null, [], \'X_X_XcontextX_X_X\', NaN)', hookName, context);
          f = template.expression;
          ...
          if (isInitialImportScripts) {
            ...
            if (hookWith && blockScope[hookWith]) {
              ...
              template = _espreeParse('_X_hookName_X_(() => require(\'module.js\'), null, [], \'X_X_XcontextX_X_X\', NaN)', hookName, context);
              f = template.expression;
    ...
    case 'MetaProperty':
      ...
      if (ast.__hooked__) {
        ...
        if (ast.meta === 'import' && ast.property === 'meta') {
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse('_X_hookName_X_(() => import.meta, null, [\'import.meta\'], \'X_X_XcontextX_X_X\', NaN)', hookName, context);
          f = template.expression;
          f.__hooked__ = true;
          f.callee.__hooked__ = true;
          f.arguments[0].__hooked__ = true;
    ...
    case 'Identifier':
      if (ast.__hooked__) {
        ...
        if (ast.__with__) {
          ...
          template.expression.property.name = t ? 'S' + _hookPrefix + ast.name : _hookPrefix + ast.name;
          template.expression.__hooked__ = true;
          ast.name = undefined;
          ast.__global__ = undefined;
          objectAssign(ast, template.expression);
      ...
      let stream = new htmlparser.WritableStream({
        ...
        onopentag(name, attributes) {
          ...
          for (let attr in attributes) {
            ...
            if (name === 'object' && attr === 'data' && attributes[attr] && hook.parameters.emptySvg) {
              ...
              case 'http:':
                ...
                _value = 'data:image/svg+xml;base64,' + btoa(hook.parameters.emptySvg.replace('$location$', new URL(attributes[attr], url).href));
                break;
            ...
            else if (attr.match(/^on[a-z]{1,}$/) && attributes[attr]) {
              ...
              if (!_attrNoHook) {
                _value = 'return ' + hook('(() => { ' + attributes[attr] + '})()',
                  hookName,
              ...
              case 'data:':
                {
                  let dataURL = _srcUrl.pathname.match(/^([a-z\/]*)(;[^, ]*)?,(.*)$/);
                  if (dataURL && dataURL[1].toLowerCase() === 'text/html') {
                    ...
                    if (hook.parameters.emptyDocumentUrl) {
                      let _emptySrc = hook.parameters.emptyDocumentUrl.origin + hook.parameters.emptyDocumentUrl.pathname +
                        '?url=' + encodeURIComponent(url.origin + url.pathname + ',iframe' + (attributes['id'] ? '@' + attributes['id'] : ''));
                      attrs += ' src="' + _emptySrc + '"';
                    }
              ...
              case 'blob:':
                {
                  if (hook.parameters.emptyDocumentUrl) {
                    let _emptySrc = hook.parameters.emptyDocumentUrl.origin + hook.parameters.emptyDocumentUrl.pathname +
                      '?url=' + encodeURIComponent(url.origin + url.pathname + ',iframe' + (attributes['id'] ? '@' + attributes['is'] : ''));
                    attrs += ' src="' + _emptySrc + '"';
                  }
                  _attr = 'onload';
