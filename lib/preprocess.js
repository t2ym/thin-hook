  ...
  function _preprocess(ast, isConstructor = false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap) {
    ...
    case 'NewExpression':
      if (metaHooking && ast.callee && ast.callee.name === 'Function') {
        context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        template = _espreeParse('new (hook.Function(\'' + hookName + '\', [[\'' + context + '\', {}]], \'' + contextGeneratorName + '\'))(\'1\');',
    ...
    case 'CallExpression':
      if (metaHooking &&
        ...
        context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        template = _espreeParse('hook.' + ast.callee.name + '(\'' + hookName + '\', [[\'' + context + '\', {}]], \'' + contextGeneratorName + '\')',
            hookName, context).body[0].expression;
        if (_sourceMap) {
          _trimStartEndRaw(template);
        }
