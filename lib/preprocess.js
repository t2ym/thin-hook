        ...
        if (ast.__for__ &&
          ...
          case 'Identifier':
            if (!blockScope[ast.declarations[0].id.name]) {
              ...
              if (t) {
                template.property.value = 'S' + _hookPrefix + body.name + ';' + context;
              }
              else {
                template.property.value = _hookPrefix + body.name + ';' + context;
              }
      ...
      if (ast.__hooked__) {
        ...
        if (_hookProperty) {
          ...
          if (_hookGlobal && Object.getPrototypeOf(functionScope) === globalScope) {
            ...
            if (t) {
              template.expression.left.property.value = 'S' + _hookPrefix + ast.id.name + ';' + context;
            }
            else {
              template.expression.left.property.value = _hookPrefix + ast.id.name + ';' + context;
            }
    ...
    case 'Identifier':
      if (ast.__hooked__) {
        ...
        if (ast.__with__) {
          ...
          template.expression.property.value = t ? 'S' + _hookPrefix + ast.name + ';' + context : _hookPrefix + ast.name + ';' + context;
          template.expression.__hooked__ = true;
          ast.name = undefined;
          ast.__global__ = undefined;
          objectAssign(ast, template.expression);
          ...
          for (let attr in attributes) {
            ...
            if (attr.match(/^on[a-z]{1,}$/) && attributes[attr]) {
              ...
              case 'blob:':
                {
                  if (hook.parameters.emptyDocumentUrl) {
                    let _emptySrc = hook.parameters.emptyDocumentUrl.pathname +
                      '?url=' + encodeURIComponent((url.origin === hook.parameters.emptyDocumentUrl.origin ? '' : url.origin) + url.pathname + ',iframe' + (attributes['id'] ? '@' + attributes['is'] : ''));
                    attrs += ' src="' + _emptySrc + '"';
                  }
                  _attr = 'onload';
              ...
              case 'http:':
                ...
                _value = 'data:image/svg+xml;base64,' + btoa(hook.parameters.emptySvg.replace('$location$',
