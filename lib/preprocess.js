          ...
          for (let attr in attributes) {
            ...
            if (attr.match(/^on[a-z]{1,}$/) && attributes[attr]) {
              ...
              if (!_attrNoHook) {
                ...
                let insertReturnAt = _value.indexOf('\']);') + 4;
                _value = _value.substring(0, insertReturnAt) + 'return ' + _value.substring(insertReturnAt);
              }
            }
            ...
            if (inlineScript) {
              ...
              if (!noHook) {
                ...
                inlineScript = inlineScript
                  ? hook(inlineScript, hookName, [[(cors ? url.href : url.pathname) + ',script@' + (scriptOffset + processed.length), {}]], contextGenerator, metaHooking, _hookProperty)
                  : '';
              }
              if (isSVG) {
