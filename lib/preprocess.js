        ...
        onopentag(name, attributes) {
          ...
          for (let attr in attributes) {
            ...
            if (attr.match(/^on[a-z]{1,}$/) && attributes[attr]) {
              if (name === 'iframe' && attr === 'onload' && typeof attributes['srcdoc'] === 'string') {
              ...
              if (!attributes[attr].trim().match(/^(([^:\/?#]+):)?(\/\/([^\/?#]*))?([^?#]*[.]m?js)(\?([^#]*))?(#(.*))?$/)) {
                // drop invalid script src URL attribute
                continue;
              }
            }
            else if (attr === 'src' && name === 'iframe' && attributes[attr] && typeof attributes['srcdoc'] === 'string') {
              continue; // skip iframe src
            }
