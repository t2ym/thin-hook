...
module.exports = function(espree, escodegen, htmlparser, createHash, convert) {
  ...
  function _preprocess(ast, isConstructor = false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap) {
    ...
    case 'MemberExpression':
      if (_hookProperty &&
        ...
        let template = _trimStartEndRaw(espree.parse(
    ...
    case 'FunctionExpression':
      if (ast.__hooked__) {
        ...
        let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        let template = _trimStartEndRaw(espree.parse(ast.generator
    ...
    case 'ArrowFunctionExpression':
      if (ast.__hooked__) {
        ...
        if (typeof ast.body === 'object' &&
            !Array.isArray(ast.body)) {
          let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          let template = _trimStartEndRaw(espree.parse(ast.body.type === 'BlockStatement'
    ...
    case 'NewExpression':
      if (metaHooking && ast.callee && ast.callee.name === 'Function') {
        let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        let template = _trimStartEndRaw(espree.parse('new (hook.Function(\'' + hookName + '\', [[\'' + context + '\', {}]], \'' + contextGeneratorName + '\'))(\'1\');',
    ...
    case 'CallExpression':
      if (metaHooking &&
        ...
        let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        let template = _trimStartEndRaw(espree.parse('hook.' + ast.callee.name + '(\'' + hookName + '\', [[\'' + context + '\', {}]], \'' + contextGeneratorName + '\')',
            espreeOptions)).body[0].expression;
        ast.callee = template;
      }
        ...
        if (ast.__hooked__) {
          ...
          let template = espree.parse('hook.' + ast.callee.name + '(\'' + hookName + '\', [[\'' + context + '\', {}]], \'' + contextGeneratorName + '\')',
              espreeOptions).body[0].expression;
          let wrapper = _trimStartEndRaw(espree.parse('(script, eval) => eval(script)', espreeOptions)).body[0].expression;
          ast.callee = template;
  ...
  function hook(code, hookName = '__hook__', initialContext = [], contextGeneratorName = 'method', metaHooking = true, _hookProperty = getHookProperty(), _sourceMap = null) {
    let targetAst;
    try {
