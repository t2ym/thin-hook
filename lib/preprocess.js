...
module.exports = function(espree, escodegen, htmlparser, createHash, convert, he) {
  ...
  function setupHookWorkers() {
    if (hookWorkers.length === 0 && hook.parameters.hookWorker) {
      ...
      let noHookAuthorization = new URL(top.document.querySelector('script').src).searchParams.has('no-hook-authorization') 
        ? new URL(top.document.querySelector('script').src).searchParams.get('no-hook-authorization')
        : null;
