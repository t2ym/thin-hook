...
module.exports = function(espree, escodegen, htmlparser, createHash) {

  const espreeOptions = { range: true, tokens: true, comment: true, ecmaVersion: 8 };
  const espreeModuleOptions = { range: true, tokens: true, comment: true, ecmaVersion: 8, sourceType: 'module' };
  ...
  function _preprocess(ast, isConstructor = false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, hookProperty) {
    switch (ast.type) {
    case 'MemberExpression':
      if (typeof ast.object === 'object' &&
        ast.object.type !== 'Super' &&
        typeof ast.property === 'object') {
        let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        let template = espree.parse(
    ...
    case 'CallExpression':
      ...
      else if (ast.callee && ast.callee.type === 'MemberExpression' && ast.callee.object.type !== 'Super') {
        let context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        let template = espree.parse(
        ...
        if (Array.isArray(ast[target])) {
          ast[target].forEach((item, index) => {
            if (item && typeof item === 'object' && typeof item.type === 'string') {
              ...
              _preprocess(item, false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, hookProperty);
              astPath.pop();
            }
          });
        }
        else if (typeof ast[target] === 'object' && typeof ast[target].type === 'string') {
          _preprocess(ast[target], isConstructor, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, hookProperty);
        }
        astPath.pop();
      }
    }
  ...
  function _preprocessHtml(html, hookName, url, cors, contextGenerator, contextGeneratorScripts, isDecoded, metaHooking = true, scriptOffset = 0, hookProperty = true) {
    let processed = '';
    let inScript = false;
    let noHook = false;
          ...
          if (attr.match(/^on[a-z]{1,}$/) && attributes[attr]) {
            ...
            if (!_attrNoHook) {
              attributes[attr] = 'return ' + hook('(() => { ' + attributes[attr] + '})()',
                ...
                + ',' + attr + '@' + (scriptOffset + processed.length), {}]], contextGeneratorName, metaHooking, 0, hookProperty);
            }
          }
          ...
          if (inlineScript) {
            ...
            if (!noHook) {
              inlineScript = hook(inlineScript, hookNameForServiceWorker,
                [[(cors ? url.href : url.pathname) + ',script@' + (scriptOffset + processed.length), {}]], contextGeneratorName, metaHooking, 0, hookProperty);
            }
          }
  ...
  function hook(code, hookName = '__hook__', initialContext = [], contextGeneratorName = 'method', metaHooking = true, hookProperty = true) {
    let targetAst;
    try {
      targetAst = espree.parse(code, espreeOptions);
    }
    catch (e) {
    ...
    _preprocess(targetAst, false, hookName, initialContext, contextGeneratorName, contextGenerator, metaHooking, hookProperty);
