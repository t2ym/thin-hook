...
module.exports = function(espree, escodegen, htmlparser, createHash, convert) {
  ...
  let configurationsJSON = '{"hookNameForServiceWorker":"__hook__","contextGeneratorName":"method","discardHookErrors":true,"hookProperty":true,"hookGlobal":true,"compact":false,"noHookAuthorizationPreValidated":[],"contextGeneratorScripts":[]}';
  ...
  function mutateScriptConfigurationsJSON(script, url) {
    if (url.href.match(/\/hook[.]min[.]js/)) {
      let mutatedScript = script.replace(
        /('{"hookNameForServiceWorker":")(__hook__)(","contextGeneratorName":")(method)(","discardHookErrors":)(true)(,"hookProperty":)(true)(,"hookGlobal":)(true)(,"compact":)(false)(,"noHookAuthorizationPreValidated":\[\],"contextGeneratorScripts":\[\]}')/,
  ...
  function _analyzeScope(ast, blockScope, functionScope, moduleScope) {
    ...
    case 'RestElement':
      ...
      if (scope) {
        ...
        if (ast.argument && ast.argument.type === 'Identifier' && ast.argument.name) {
          ...
          console.error(type + ': Missing Argument Identifier');
        }
      }
      break;
        ...
        if (Array.isArray(child)) {
          for (t = child, index = 0, l = t.length; index < l; index++) {
            ...
            if (item instanceof Object && typeof item.type === 'string') {
              _analyzeScope(item, blockScope, functionScope, moduleScope);
            }
          }
        }
        else if (child instanceof Object && typeof child.type === 'string') {
          _analyzeScope(child, blockScope, functionScope, moduleScope);
        }
      }
    }
  }
  ...
  function _preprocess(ast, isConstructor = false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope){
      ...
      if (ast.__hooked__) {
        ...
        if (_hookProperty && _hookGlobal) {
          ...
          if (Object.getPrototypeOf(functionScope) === globalScope) {
            ...
            template.expression.left.property.name = ast.id.name;
      ...
      if (ast.__hooked__) {
        ...
        if (ast.__hooked__) {
          ...
          if (!isInitialImportScripts) {
            ...
            template.expression.property.name = ast.callee.name;
            template.expression.__hooked__ = true;
    ...
    case 'Identifier':
      if (ast.__hooked__) {
        ...
        template.expression.property.name = ast.name;
        template.expression.__hooked__ = true;
        delete ast.name;
        delete ast.__global__;
        Object.assign(ast, template.expression);
        ...
        for (index = 0, l = ast.elements.length; index < l; index++) {
          if (ast.elements[index]) {
            ...
            case 'ObjectPattern':
              ...
              break;
            case 'RestElement':
              // TODO: How to handle RestElement in ArrayPattern?
              // var [...hook.global().b] = [ 1, 2, 3 ]; // Syntax Error: Unexpected .
              // var [...[hook.global().c]] = [ 1, 2, 3 ]; // Syntax Error: Unexpected .
              //console.warn(ast.type + ': Skip hooking RestElement in ArrayPattern to avoid Syntax Error');
              break;
            default:
        ...
        if (Array.isArray(child)) {
          for (t = child, index = 0, l = t.length; index < l; index++) {
            ...
            if (item && typeof item === 'object' && typeof item.type === 'string') {
              ...
              _preprocess(item, false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
              astPath.pop();
            }
          }
        }
        else if (typeof child === 'object' && typeof child.type === 'string') {
          _preprocess(child, isConstructor, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
        }
        astPath.pop();
      }
    }
  ...
  function hook(code, hookName = '__hook__', initialContext = [], contextGeneratorName = 'method', metaHooking = true, _hookProperty = getHookProperty(), _sourceMap = null, asynchronous = false, _compact = getCompact(), _hookGlobal = getHookGlobal(), initialScope = null) {
    if (asynchronous) {
      let worker = getBestHookWorker();
      if (worker) {
        return new Promise(function (resolve, reject) {
          ...
          let message = [ id, 'text/javascript', code, hookName, initialContext, contextGeneratorName, metaHooking, _hookProperty, _sourceMap, false, _compact, _hookGlobal, initialScope ];
          worker.port.postMessage(JSON.stringify(message, null, 0));
        ...
        return new Promise(function (resolve, reject) {
          try {
            resolve(hook(code, hookName, initialContext, contextGeneratorName, metaHooking, _hookProperty, _sourceMap, false, _compact, _hookGlobal, initialScope));
          }
          catch (e) {
            reject(e);
          }
      ...
      _analyzeScope(targetAst, blockScope, functionScope, globalScope);
      ...
      _preprocess(targetAst, false, hookName, initialContext, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
      //_logScope(targetAst);
