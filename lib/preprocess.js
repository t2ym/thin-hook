...
module.exports = function(espree, escodegen, htmlparser, createHash, convert) {
  ...
  const nonOverridableIdentifiers = Object.assign(Object.create(null), {
    undefined: '__unexpected_overridden_declaration_of_undefined__',
    NaN: '__unexpected_overridden_declaration_of_NaN__',
    Infinity: '__unexpected_overridden_declaration_of_Infinity__',
  });
  ...
  function _analyzeScope(ast, blockScope, functionScope, moduleScope, hookName, hookWith) {
    ...
    case 'CatchClause':
      ...
      case 'FunctionDeclaration':
        if (ast.id && ast.id.name) {
          ast.id.name = nonOverridableIdentifiers[ast.id.name] || ast.id.name;
    ...
    case 'Identifier':
      ...
      if (scope) {
        if (ast.name) {
          ast.name = nonOverridableIdentifiers[ast.name] || ast.name;
    ...
    case 'AssignmentPattern':
      ...
      if (scope) {
        if (ast.left.name) {
          ast.left.name = nonOverridableIdentifiers[ast.left.name] || ast.left.name;
        ...
        if (ast.argument) {
          ...
          case 'Identifier':
            if (ast.argument.name) {
              ast.argument.name = nonOverridableIdentifiers[ast.argument.name] || ast.argument.name;
        ...
        if (ast.specifiers[index]) {
          ...
          case 'ImportSpecifier':
            if (ast.specifiers[index].local && ast.specifiers[index].local.type === 'Identifier' && ast.specifiers[index].local.name) {
              ...
              local.name = nonOverridableIdentifiers[local.name] || local.name;
