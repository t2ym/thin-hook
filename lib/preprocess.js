...
module.exports = function(espree, escodegen, htmlparser, createHash, convert) {
  ...
  function _espreeParse(code, hookName, context) {
    if (!_espreeCache[code]) {
      _espreeCache[code] = JSON.stringify(espree.parse(code, espreeOptions), null, 0);
    }
      ...
      if (ast.__hooked__) {
        ...
        template = _espreeParse(ast.generator
          ...
          : isConstructor
            ...
            : 'function f() { return _X_hookName_X_(() => {}, this, arguments, \'X_X_XcontextX_X_X\'); }', hookName, context).body[0];
        f = ast.generator
        ...
        if (typeof ast.body === 'object' &&
          ...
          template = _espreeParse(ast.body.type === 'BlockStatement'
            ...
            : ast.body.type === 'ObjectExpression'
              ...
              : '(...args) => _X_hookName_X_(p => p, this, args, \'X_X_XcontextX_X_X\')', hookName, context).body[0].expression;
    ...
    case 'CallExpression':
      if (metaHooking &&
        ...
        if (ast.__hooked__) {
          ...
          template = _espreeParse('hook.' + ast.callee.name + '(\'_X_hookName_X_\', [[\'X_X_XcontextX_X_X\', {}]], \'' + contextGeneratorName + '\')',
              hookName, context).body[0].expression;
          wrapper = _espreeParse('(script, eval) => eval(script)', hookName, context).body[0].expression;
          if (_sourceMap) {
            _trimStartEndRaw(template);
          }
          ast.callee = template;
