...
module.exports = function(espree, escodegen, htmlparser, createHash) {
  ...
  function setScriptHookProperty(script, url) {
    if (url.href.match(/\/hook[.]min[.]js/)) {
      script = script.replace(/let[ ]hookProperty[ ]?=[ ]?[^;]*;/g, 'let' + ' hookProperty = ' + getHookProperty() + ';')
    }
    return script;
  }

  function _preprocess(ast, isConstructor = false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty) {
    switch (ast.type) {
        ...
        if (Array.isArray(ast[target])) {
          ast[target].forEach((item, index) => {
            if (item && typeof item === 'object' && typeof item.type === 'string') {
              ...
              _preprocess(item, false, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty);
              astPath.pop();
            }
          });
        }
        else if (typeof ast[target] === 'object' && typeof ast[target].type === 'string') {
          _preprocess(ast[target], isConstructor, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty);
        }
        astPath.pop();
      }
    }
  ...
  function _preprocessHtml(html, hookName, url, cors, contextGenerator, contextGeneratorScripts, isDecoded, metaHooking = true, scriptOffset = 0, _hookProperty = true) {
    let processed = '';
    let inScript = false;
    let noHook = false;
  ...
  function hook(code, hookName = '__hook__', initialContext = [], contextGeneratorName = 'method', metaHooking = true, _hookProperty = getHookProperty()) {
    let targetAst;
    try {
    ...
    _preprocess(targetAst, false, hookName, initialContext, contextGeneratorName, contextGenerator, metaHooking, _hookProperty);
