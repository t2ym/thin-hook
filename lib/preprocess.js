...
module.exports = function(espree, escodegen, htmlparser, createHash, convert) {
  ...
  const espreeOptionsForTarget = { loc: true, range: true, tokens: true, comment: true, ecmaVersion: 8, ecmaFeatures: { experimentalObjectRestSpread: true } };
  const espreeOptionsForTargetCompact = { loc: false, range: false, tokens: false, comment: false, ecmaVersion: 8, ecmaFeatures: { experimentalObjectRestSpread: true } };
  const espreeOptions = { loc: false, range: false, tokens: false, comment: false, ecmaVersion: 8, ecmaFeatures: { experimentalObjectRestSpread: true } };
  const espreeModuleOptions = { loc: true, range: true, tokens: true, comment: true, ecmaVersion: 8, sourceType: 'module', ecmaFeatures: { experimentalObjectRestSpread: true } };
  const espreeModuleOptionsCompact = { loc: false, range: false, tokens: false, comment: false, ecmaVersion: 8, sourceType: 'module', ecmaFeatures: { experimentalObjectRestSpread: true } };
  ...
  function _analyzeScope(ast, blockScope, functionScope, moduleScope, hookName) {
        ...
        for (i = 0; i < ast.properties.length; i++) {
          if (ast.properties[i]) {
            if (ast.properties[i].value) {
              ...
              switch (ast.properties[i].value.type) {
              case 'Identifier':
              case 'AssignmentPattern':
              case 'ArrayPattern':
              case 'ObjectPattern':
              ...
              ast.properties[i].argument.__scope__ = scope;
            }
          }
        }
      }
        ...
        if (ast.specifiers[index]) {
          ...
          case 'ImportSpecifier':
            if (ast.specifiers[index].local && ast.specifiers[index].local.type === 'Identifier' && ast.specifiers[index].local.name) {
              functionScope[ast.specifiers[index].local.name] = type;
            }
            break;
          default:
            break;
        ...
        if (Array.isArray(child)) {
          for (t = child, index = 0, l = t.length; index < l; index++) {
            ...
            if (item instanceof Object && typeof item.type === 'string') {
              _analyzeScope(item, blockScope, functionScope, moduleScope, hookName);
            }
          }
        }
        else if (child instanceof Object && typeof child.type === 'string') {
          _analyzeScope(child, blockScope, functionScope, moduleScope, hookName);
        }
      }
    }
  }
  ...
  function _preprocess(ast, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, _hookPrefix, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope){
    let context, template, f, params, body, useStrict, wrapper, target, child, t, index, l, item;
    ...
    case 'VariableDeclaration':
      if (_hookProperty && _hookGlobal &&
        ...
        delete ast.kind;
        Object.assign(ast, template);
      }
      break;
    case 'VariableDeclarator':
      if (_hookProperty && _hookGlobal && ast.id && typeof ast.id === 'object') {
        if (ast.init) {
          if (ast.init.type === 'Identifier' && ast.init.name && !blockScope[ast.init.name]) {
    ...
    case 'ClassExpression':
      if (ast.__hooked__) {
        ...
        if (ast.superClass && ast.superClass.type === 'Identifier' && ast.superClass.name && !blockScope[ast.superClass.name]) {
    ...
    case 'ClassDeclaration':
      if (ast.__hooked__) {
        ...
        if (_hookProperty && _hookGlobal) {
          ...
          if (Object.getPrototypeOf(functionScope) === globalScope) {
            context = contextGenerator(astPath).replace(/\'/g, '\\\'');
            template = _espreeParse(
              'hook.global(_X_hookName_X_, \'X_X_XcontextX_X_X\', \'cname\', \'class\').cname = class cname {}',
              hookName, context);
            ...
            template.expression.left.property.name = _hookPrefix + ast.id.name;
            template.expression.left.__hooked__ = true;
            Object.assign(template.expression.right, ast);
    ...
    case 'MemberExpression':
      if (ast.__hooked__) {
        ...
        if (ast.object.type === 'Super') {
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse(
            '_X_hookName_X_(\'s.\', this, [\'p\', p => super[p]], \'X_X_XcontextX_X_X\')',
            hookName, context);
      ...
      if (ast.__hooked__) {
        ...
        case 'MemberExpression':
          if (ast.argument.object.type === 'Super') {
            context = contextGenerator(astPath).replace(/\'/g, '\\\'');
            template = _espreeParse(
              '_X_hookName_X_(\'op\', this, [\'p\', p => super[p]++], \'X_X_XcontextX_X_X\')',
              hookName, context);
            f = template.expression;
            params = f.arguments[2].elements;
          ...
          break;
        case 'Identifier':
          if (_hookGlobal && ast.argument.name && !blockScope[ast.argument.name]) {
      ...
      if (_hookProperty &&
        ...
        case 'Identifier':
          if (_hookGlobal) {
            switch (ast.operator) {
            case 'delete':
              if (ast.argument.name && !blockScope[ast.argument.name]) {
                context = contextGenerator(astPath).replace(/\'/g, '\\\'');
                template = _espreeParse(
                  'hook.global(_X_hookName_X_, \'X_X_XcontextX_X_X\', \'name\', \'delete\')',
                  hookName, context);
                template.expression.arguments[2].value = ast.argument.name;
                template.expression.__hooked__ = true;
                template.expression.callee.__hooked__ = true;
                delete ast.argument;
                Object.assign(ast, template.expression);
              }
              break;
            case 'typeof':
              if (ast.argument.name && !blockScope[ast.argument.name]) {
    ...
    case 'AssignmentExpression':
      if (ast.__hooked__) {
        ...
        if (_hookGlobal && ast.right.type === 'Identifier' && !blockScope[ast.right.name]) {
        ...
        case 'MemberExpression':
          if (ast.left.object.type === 'Super') {
            context = contextGenerator(astPath).replace(/\'/g, '\\\'');
            template = _espreeParse(
              '_X_hookName_X_(\'op\', this, [\'p\', v, (p, v) => super[p] = v], \'X_X_XcontextX_X_X\')',
              hookName, context);
            f = template.expression;
            params = f.arguments[2].elements;
          ...
          break;
        case 'Identifier':
          if (_hookGlobal && !blockScope[ast.left.name]) {
    ...
    case 'MethodDefinition':
      if (_hookProperty && _hookGlobal) {
        if (ast.computed && ast.key && ast.key.type === 'Identifier' && ast.key.name && !blockScope[ast.key.name]) {
    ...
    case 'FunctionExpression':
      if (ast.__hooked__) {
        ...
        template = _espreeParse(t
          ? ast.generator
            ? 'function * f(...args) { yield * _X_hookName_X_(function * () {}, this, args, \'X_X_XcontextX_X_X\'); }'
        ...
        if (_hookProperty && ast.type === 'FunctionDeclaration' && Object.getPrototypeOf(functionScope) === globalScope) {
          template = _espreeParse(
            'hook.global(_X_hookName_X_, \'X_X_XcontextX_X_X\', \'fname\', \'function\').fname = function fname() {}',
            hookName, context);
          template.expression.left.object.arguments[2].value = ast.id.name;
          template.expression.left.object.__hooked__ = true;
          template.expression.left.object.callee.__hooked__ = true;
      ...
      if (ast.__hooked__) {
        ...
        if (typeof ast.body === 'object' &&
          ...
          template = _espreeParse(ast.body.type === 'BlockStatement'
            ? '(...args) => _X_hookName_X_(p => { return p; }, null, args, \'X_X_XcontextX_X_X\')'
            : ast.body.type === 'ObjectExpression'
              ? '(...args) => _X_hookName_X_(p => ({ p: p }), null, args, \'X_X_XcontextX_X_X\')'
              : '(...args) => _X_hookName_X_(p => p, null, args, \'X_X_XcontextX_X_X\')', hookName, context).expression;
    ...
    case 'NewExpression':
      ...
      else if (metaHooking && ast.callee && ast.callee.name === 'Function') {
        context = contextGenerator(astPath).replace(/\'/g, '\\\'');
    ...
    case 'CallExpression':
      if (ast.__hooked__) {
        ...
        if (ast.__hooked__) {
          ...
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse('hook.' + ast.callee.name + '(\'_X_hookName_X_\', [[\'X_X_XcontextX_X_X\', {}]], \'' + contextGeneratorName + '\', {v:true})',
              hookName, context).expression;
      ...
      else if (_hookProperty && _hookGlobal && ast.callee && ast.callee.type === 'Identifier' && !blockScope[ast.callee.name]) {
        if (ast.__hooked__) {
          delete ast.__hooked__;
        }
        else {
          ...
          if (!isInitialImportScripts) {
            context = contextGenerator(astPath).replace(/\'/g, '\\\'');
            template = _espreeParse(
              'hook.global(_X_hookName_X_, \'X_X_XcontextX_X_X\', \'name\', \'get\').name',
              hookName, context);
            template.expression.object.arguments[2].value = ast.callee.name;
            template.expression.object.__hooked__ = true;
            template.expression.object.callee.__hooked__ = true;
            template.expression.property.name = _hookPrefix + ast.callee.name;
            template.expression.__hooked__ = true;
            delete ast.callee.name;
            Object.assign(ast.callee, template.expression);
          }
        }
      }
        ...
        if (ast.callee.type === 'Super') {
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse('_X_hookName_X_((...args) => super(...args), null, [], \'X_X_XcontextX_X_X\', 0)', hookName, context);
          f = template.expression;
          f.arguments[2].elements = ast.arguments;
          f.__hooked__ = true;
          ...
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse('_X_hookName_X_(f, null, [], \'X_X_XcontextX_X_X\', 0)', hookName, context);
          f = template.expression;
          f.arguments[0] = ast.callee;
          f.arguments[2].elements = ast.arguments;
          f.__hooked__ = true;
    ...
    case 'Identifier':
      ...
      else if (_hookProperty && _hookGlobal && ast.__global__ && ast.name && !blockScope[ast.name]) {
        context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        template = _espreeParse(
          'hook.global(_X_hookName_X_, \'X_X_XcontextX_X_X\', \'name\', \'var\').name',
          hookName, context);
        template.expression.object.arguments[2].value = ast.name;
        template.expression.object.arguments[3].value = ast.__global__;
        template.expression.object.__hooked__ = true;
        template.expression.object.callee.__hooked__ = true;
        template.expression.object.callee.object.__hooked__ = true;
        template.expression.object.callee.property.__hooked__ = true;
        template.expression.property.name = _hookPrefix + ast.name;
        template.expression.__hooked__ = true;
        delete ast.name;
        delete ast.__global__;
        Object.assign(ast, template.expression);
      ...
      break;
    case 'ArrayPattern':
      if (_hookProperty && _hookGlobal && ast.__global__) {
    ...
    case 'ObjectPattern':
      ...
      else if (_hookProperty && _hookGlobal && ast.__global__) {
        for (index = 0, l = ast.properties.length; index < ast.properties.length; index++) {
          if (ast.properties[index]) {
            if (ast.properties[index].value) {
              ...
              case 'ObjectPattern':
              case 'MemberExpression':
                if (ast.properties[index].shorthand && ast.properties[index].value.type === 'Identifier' && !blockScope[ast.properties[index].value.name]) {
    ...
    case 'Property':
      if (_hookProperty && _hookGlobal) {
        ...
        if (ast.value && ast.value.type === 'Identifier' && !ast.value.__global__ && ast.value.name && !blockScope[ast.value.name]) {
    ...
    case 'ExperimentalSpreadProperty':
      if (_hookProperty) {
        if (_hookGlobal) {
          if (ast.argument && ast.argument.type === 'Identifier' && ast.argument.name && !blockScope[ast.argument.name]) {
            ast.argument.__global__ = 'get';
          }
        }
        context = contextGenerator(astPath).replace(/\'/g, '\\\'');
        template = _espreeParse(
          '_X_hookName_X_(\'*\', o, [], \'X_X_XcontextX_X_X\')',
          hookName, context);
        f = template.expression;
    ...
    case 'AssignmentPattern':
      if (_hookProperty && _hookGlobal) {
        ...
        if (ast.right && ast.right.type === 'Identifier' && ast.right.name && !blockScope[ast.right.name]) {
          ast.right.__global__ = 'get';
        }
      }
      break;
    case 'ExpressionStatement':
      if (_hookProperty && _hookGlobal) {
        if (ast.expression && ast.expression.type === 'Identifier' && ast.expression.name && !blockScope[ast.expression.name]) {
    ...
    case 'BinaryExpression':
      if (_hookProperty) {
        if (_hookGlobal) {
          if (ast.left && ast.left.type === 'Identifier' && ast.left.name && !blockScope[ast.left.name]) {
        ...
        if (ast.operator === 'in') {
          context = contextGenerator(astPath).replace(/\'/g, '\\\'');
          template = _espreeParse(
            '_X_hookName_X_(\'in\', o, [\'p\'], \'X_X_XcontextX_X_X\')',
            hookName, context);
          f = template.expression;
          params = f.arguments[2].elements;
    ...
    case 'ConditionalExpression':
      if (_hookProperty && _hookGlobal) {
        if (ast.test && ast.test.type === 'Identifier' && ast.test.name && !blockScope[ast.test.name]) {
          ast.test.__global__ = 'get';
        }
        if (ast.consequent && ast.consequent.type === 'Identifier' && ast.consequent.name && !blockScope[ast.consequent.name]) {
          ast.consequent.__global__ = 'get';
        }
        if (ast.alternate && ast.alternate.type === 'Identifier' && ast.alternate.name && !blockScope[ast.alternate.name]) {
    ...
    case 'ExportDefaultDeclaration':
      if (_hookProperty && _hookGlobal) {
        if (ast.declaration && ast.declaration.type === 'Identifier' && ast.declaration.name && !blockScope[ast.declaration.name]) {
    ...
    case 'ForStatement':
      if (_hookProperty && _hookGlobal) {
        if (ast.init && ast.init.type === 'Identifier' && ast.init.name && !blockScope[ast.init.name]) {
          ast.init.__global__ = 'get';
        }
        if (ast.test && ast.test.type === 'Identifier' && ast.test.name && !blockScope[ast.test.name]) {
          ast.test.__global__ = 'get';
        }
        if (ast.update && ast.update.type === 'Identifier' && ast.update.name && !blockScope[ast.update.name]) {
    ...
    case 'SwitchStatement':
      if (_hookProperty && _hookGlobal) {
        if (ast.discriminant && ast.discriminant.type === 'Identifier' && ast.discriminant.name && !blockScope[ast.discriminant.name]) {
    ...
    case 'TaggedTemplateExpression':
      if (_hookProperty && _hookGlobal) {
        if (ast.tag && ast.tag.type === 'Identifier' && ast.tag.name && !blockScope[ast.tag.name]) {
    ...
    case 'TemplateLiteral':
      if (_hookProperty && _hookGlobal) {
        for (index = 0, l = ast.expressions.length; index < l; index++) {
          if (ast.expressions[index] && !ast.expressions[index].__global__ && ast.expressions[index].type === 'Identifier' && !blockScope[ast.expressions[index].name]) {
    ...
    case 'WithStatement':
      if (_hookProperty && _hookGlobal) {
        if (ast.object && ast.object.type === 'Identifier' && ast.object.name && !blockScope[ast.object.name]) {
        ...
        if (Array.isArray(child)) {
          for (t = child, index = 0, l = t.length; index < l; index++) {
            ...
            if (item && typeof item === 'object' && typeof item.type === 'string') {
              ...
              _preprocess(item, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, _hookPrefix, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
              astPath.pop();
            }
          }
        }
        else if (typeof child === 'object' && typeof child.type === 'string') {
          _preprocess(child, hookName, astPath, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, _hookPrefix, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
        }
        astPath.pop();
      }
    }
        ...
        onclosetag(name) {
          if (name === 'script' && inScript) {
            ...
            if (inlineScript) {
              ...
              if (!noHook) {
                inlineScript = hook(inlineScript, hookName,
  ...
  function hook(code, hookName = '__hook__', initialContext = [], contextGeneratorName = 'method', metaHooking = true, _hookProperty = getHookProperty(), _sourceMap = null, asynchronous = false, _compact = getCompact(), _hookGlobal = getHookGlobal(), _hookPrefix = getHookPrefix(), initialScope = null) {
    if (asynchronous) {
      let worker = getBestHookWorker();
      if (worker) {
        return new Promise(function (resolve, reject) {
          ...
          let message = [ id, 'text/javascript', code, hookName, initialContext, contextGeneratorName, metaHooking, _hookProperty, _sourceMap, false, _compact, _hookGlobal, _hookPrefix, initialScope ];
          worker.port.postMessage(JSON.stringify(message, null, 0));
      ...
      _analyzeScope(targetAst, blockScope, functionScope, globalScope, hookName);
      ...
      _preprocess(targetAst, hookName, initialContext, contextGeneratorName, contextGenerator, metaHooking, _hookProperty, _sourceMap, _hookGlobal, _hookPrefix, blockScope, functionScope, globalScope, globalObjectScope, globalClassScope);
      //_logScope(targetAst);
