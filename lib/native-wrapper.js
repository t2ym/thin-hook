...
module.exports = function (hook, preprocess) {
    ...
    let _eval = function (script, evalWrapper = (script, _eval) => _eval(script)) {
      if (!evalWrapper || typeof evalWrapper !== 'function' ||
        ...
        script = hook(script,
          ...
          preprocess.getCompact(), preprocess.getHookGlobal(), initialScope);
      }
  ...
  function hookGlobal(hookCallback, context, name, type) {
    let proxy = {
      get [name]() {
        let value = hookCallback('.', _global, [name], context);
        if (typeof value === 'function') {
          let prototype = value.prototype;
          value = value.bind(_global);
          value.prototype = prototype;
        }
        return value;
      },
      set [name](value) {
        return hookCallback('=', _global, [name, value], context);
      }
    };
    let status = 'var';
    let descriptor;
    ...
    if (descriptor) {
      ...
      (function globalClassScopeCheck() {
        try {
          if (_globalVariables.hasOwnProperty(name)) {
            status = 'let';
          }
          else {
            _globalVariables[name] = (new Function('return ' + name))();
            status = 'let';
          }
        }
        catch (e) {
    ...
    */
    ...
    case 'undefined-class':
      descriptor = { configurable: false, enumerable: true, writable: true, value: undefined };
      Object.defineProperty(_global, name, descriptor);
      //hookCallback('()', Object, [ 'defineProperty', [ _global, name, descriptor ] ], context);
      break;
    case 'undefined-const':
      descriptor = { configurable: false, enumerable: true, writable: true, value: undefined };
      Object.defineProperty(_global, name, descriptor);
      //hookCallback('()', Object, [ 'defineProperty', [ _global, name, descriptor ] ], context);
      _globalConstVariables[name] = true;
      break;
    case 'undefined-set':
      // implicit definition
      descriptor = { configurable: true, enumerable: true, writable: true, value: undefined };
      Object.defineProperty(_global, name, descriptor);
      //hookCallback('()', Object, [ 'defineProperty', [ _global, name, descriptor ] ], context);
      break;
    case 'var-var':
    ...
    case 'let-set':
    case 'let-typeof':
      // highly discouraged for performance; all the hooked scripts should use the emulated global class scope
      proxy = _global;
      /*
      {
        get [name]() {
          return hookCallback(hook.globalGetter, _global, [name, _globalVariables], context);
        },
        set [name](value) {
          return hookCallback(hook.globalSetter, _global, [name, value, _globalVariables], context);
        }
      };
      */
      break;
  ...
  // 1000 times slower than direct variable access; Avoid using it for global variables bound to the global object
  function globalGetter(name, globalVariables) {
    if (globalVariables instanceof Object) {
      return globalVariables[name] = (new Function('return ' + name))(); // cache the value
    }
    else {
  ...
  // 1000 times slower than direct variable access; Avoid using it for global variables bound to the global object
  function globalSetter(name, value, globalVariables) {
    (new Function('$$$$value$$$$', 'return ' + name + '=' + '$$$$value$$$$'))(value);
    if (globalVariables instanceof Object) {
      globalVariables[name] = value; // cache the value
    }
    return value;
  };
