...
module.exports = function (hook, preprocess, he) {
  ...
  function hookImportScripts() {
    return function importScripts(...args) {
      ...
      // TODO: ABAC
      return _global[preprocess.getHookNameForServiceWorker()](_native.importScripts, _global, _args, 'importScripts');
    }
  }
  ...
  const hookedHTMLs = new Set();
  const allowedScriptDataURLs = new Set(); // for HTML Imports polyfill
  ...
  function hookNode(hookName, initialContext = [['Node', {}]], contextGenerator) {
    if (_native.Node && Object.getOwnPropertyDescriptor(_native.Node.prototype, 'textContent').configurable) {
      Object.defineProperty(_native.Node.prototype, 'textContent', {
        ...
        get: function () {
          let value = _nativeMethods.Node.proto.textContent.get.call(this);
          if (this instanceof _native.HTMLScriptElement) {
            hookedScripts.add(value);
          }
          return value;
        },
        set: function (value) {
          if (this instanceof _native.HTMLScriptElement) {
            if (value && !hookedScripts.has(value)) { // hook non-empty non-hooked scripts
  ...
  function hookElement(hookName, initialContext = [['Element', {}]], contextGenerator) {
          ...
          case 'src':
            if (this instanceof _native.HTMLScriptElement) {
              if (_value) {
                ...
                case 'data:': // TODO: handle data
                  if (value.startsWith('data:text/javascript;charset=utf-8,')) {
                    // Is HTML Imports polyfill?
                    let valueContent = decodeURIComponent(value.substring(35 /* 'data:text/javascript;charset=utf-8,'.length */));
                    let currentContent = this.textContent;
                    if (currentContent && valueContent.startsWith(currentContent)) {
                      let valueTail = valueContent.substring(currentContent.length);
                      let match = valueTail.match(/^\n\/\/# sourceURL=[^ ]*[.]js\n$/g);
                      if (match) {
                        // same content with sourceURL
                        allowedScriptDataURLs.add(value); // register the data URL
                        //console.log('native-wrapper.js: allow data URL for ' + valueTail.substring(1, valueTail.length - 1));
                        break; // allow to set the value as an exception
                      }
                      else {
                        //console.log('native-wrapper.js: allow unmatching data URL with tail "' + valueTail + '"');
                        break; // allow to set the value as an exception
                      }
                    }
                    else if (allowedScriptDataURLs.has(value)) {
                      // allowed data: URL
                      //console.log('native-wrapper.js: allow registered data URL');
                      break;
                    }
                  }
                ...
                case 'data:': // TODO: original onload attribute has to be called only once, while called twice
                  {
                    ...
                    if (dataURL && dataURL[1] === 'text/html') {
                      ...
                      if (hook.parameters.emptyDocumentUrl) {
                        value = hook.parameters.emptyDocumentUrl.origin + hook.parameters.emptyDocumentUrl.pathname +
                          '?url=' + encodeURIComponent(url.origin + url.pathname + ',iframe' + (this.id ? '@' + this.id : ''));
                      }
              ...
              if (!value) {
                ...
                //_nativeMethods.Element.proto.removeAttribute.value.call(this, 'src');
                setTimeout(function () { this.removeAttribute('src') }.bind(this), 0);
                return;
              }
            }
              ...
              case 'http:':
                ...
                value = 'data:image/svg+xml;base64,' + btoa(hook.parameters.emptySvg.replace('$location$', new URL(value, url).href));
                break;
    ...
    if (_native.Element && Object.getOwnPropertyDescriptor(_native.Element.prototype, 'innerHTML').configurable) {
      Object.defineProperty(_native.Element.prototype, 'innerHTML', {
        ...
        set: function (value) {
          let processed;
          if (hookedHTMLs.has(value)) {
            processed = value; // avoid multiple hooking
          }
          else if (hook.parameters.HTMLImportsXHRResponses instanceof Set && hook.parameters.HTMLImportsXHRResponses.has(value)) {
            processed = value; // avoid multiple hooking
            hookedHTMLs.add(processed);
          }
          else {
            processed = preprocess._preprocessHtml(
              value,
              hookName,
          ...
          // TODO: ABAC
          return _global[hookName](_nativeMethods.Element.proto.innerHTML.set, this, [processed], this.constructor.name + ',set innerHTML');
        }
      });
    }
    ...
    if (_native.HTMLScriptElement &&
      ...
      Object.defineProperty(_native.HTMLScriptElement.prototype, 'src', {
        ...
        set: function (value) {
          ...
          let result = _global[hookName](_nativeMethods.HTMLScriptElement.proto.src.set, this, [value], this.constructor.name + ',set src');
          return result;
        }
      });
    }
    if (_native.HTMLScriptElement && Object.getOwnPropertyDescriptor(_native.HTMLScriptElement.prototype, 'text').configurable) {
      Object.defineProperty(_native.HTMLScriptElement.prototype, 'text', {
        ...
        set: function (value) {
          ...
          return _global[hookName](_nativeMethods.HTMLScriptElement.proto.text.set, this, [value], this.constructor.name + ',set text');
        }
      });
    }
    ...
    if (_native.HTMLIFrameElement &&
      ...
      Object.defineProperty(_native.HTMLIFrameElement.prototype, 'src', {
        ...
        set: function (value) {
          ...
          _global[hookName](_nativeMethods.HTMLIFrameElement.proto.src.set, this, [value], this.constructor.name + ',set src');
        }
      });
    }
      ...
      Object.defineProperty(_native.Document.prototype, 'writeln', {
        ...
        value: function writeln(markup) {
          ...
          let processed = preprocess._preprocessHtml(
            ...
            scriptOffset,
            preprocess.getHookProperty());
          return _global[hookName](_nativeMethods.Document.proto.writeln.value, this, [processed], this.constructor.name + ',writeln');
        }
      });
    }
  ...
  function hookGlobal(hookCallback, context, name, type) {
    ...
    case 'undefined-class':
      hookCallback(strictModePrefix + '()', Object, [ 'defineProperty', [ _global, name, { configurable: false, enumerable: true, writable: true, value: undefined } ] ], context);
    ...
    case 'undefined-set':
      if (strictModePrefix) {
        ...
        // implicit definition
        hookCallback(strictModePrefix + '()', Object, [ 'defineProperty', [ _global, name, { configurable: true, enumerable: true, writable: true, value: undefined } ] ], context);
        if (newProxyDescriptor) {
          Object.defineProperty(proxy, _hookPrefix + name, newProxyDescriptor);
        }
      }
    ...
    case 'undefined-const':
      Object.defineProperty(proxy, _hookPrefix + name, {
        ...
        set: function (value) {
          hookCallback(strictModePrefix + '=', _global, [name, value], context);
          hookCallback(strictModePrefix + '()', Object, [ 'defineProperty', [ _global, name, { configurable: false, enumerable: true, writable: false, value: value } ] ], context);
        }
      });
      break;
    ...
    case 'var-delete':
      proxy = hookCallback(strictModePrefix + 'delete', _global, [name], context);
      break;
    default:
